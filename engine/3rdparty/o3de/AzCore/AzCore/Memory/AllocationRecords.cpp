...
namespace AZ::Debug
{
        ...
        unsigned int stackSuppressCount)
    {
        ...
        do
        {
            currentRequestedBytePeak = m_requestedBytesPeak.load(std::memory_order_relaxed);
            newRequestedBytePeak = AZStd::GetMax(currentRequestedBytePeak, m_requestedBytes.load(std::memory_order_relaxed));
